...
Every generation is automatically tagged with AI-powered semantic labels (subject, style, mood, use-case). Use `fairstack tags list <id>` or `GET /v1/assets/:id/tags` to see them. Add your own tags with `fairstack tags add <id> --tags "narration, onboarding"` or `POST /v1/assets/:id/tags`.
